...
async def notify_weather(channel, location, stop):
    with open('namedays_showed.json', 'r', encoding='utf-8') as file:
        namedays_showed = json.load(file)   
    today = datetime.now().strftime("%m-%d")
    if today not in namedays_showed:  
        ...
        async with python_weather.Client(unit=python_weather.METRIC, locale=python_weather.Locale.LATVIAN) as client:
            ...
            for daily in weather.daily_forecasts:
                if i == 0:
                    weather_today_title = f"📅  **Šodienas prognoze {location} {daily.date}**:\n"
                    weather_today += "🕒  *Stundas prognoze*:\n"
                    i = 1
                    ...
                    for hourly in daily.hourly_forecasts:
                        ...
                        if start_time <= hourly_time <= time(23, 59):
                            if hourly_time < time(12, 0):  # Morning (6 AM - 11:59 AM)
                                emoji_time = "🌅"
                            elif hourly_time < time(18, 0):  # Afternoon (12 PM - 5:59 PM)
                                emoji_time = "☀️"
                            else:  # Evening (6 PM - 11:59 PM)
                                emoji_time = "🌇"

                            # Format the time to show only hours and minutes
                            formatted_time = hourly_time.strftime("%H:%M")

                            weather_today += (
                                f"  {emoji_time} Laiks: {formatted_time},   Temperatūra: **{hourly.temperature}°C**, "
                                f"  Laikapstākļi: **{hourly.description}**\n"
                            )
                        
                # ⏭️  For the next day, include only times up to 6:00 AM
                else:
                    for hourly in daily.hourly_forecasts:
                        hourly_time = hourly.time  # Assuming this returns a datetime.time object
                        ...
                        if time(0, 0) <= hourly_time < end_time:
                            emoji_time = "🌙"  # Night time (12 AM - 5:59 AM)
            ...
            embed = discord.Embed(
                title=weather_today_title,  # Set the title
                description=forecastMsg,  # Set the video title in the description
                color=discord.Color.orange()  # Choose any color you'd like
            )                  
